...
try {
  ...
  await hold(3000);
  await page.screenshot({path:'artifacts/recordings/maps-live-v2-final.png'});
} finally {await context.close();await video.saveAs('artifacts/recordings/maps-live-v2.webm');await browser.close();}
writeFileSync('artifacts/recordings/maps-live-v2.ndjson',trace);
writeFileSync('artifacts/recordings/maps-live-v2.json',JSON.stringify({source:'Live Google Maps search, no preloaded results or restaurant pages; browser processes warm',preparation:initial.page.preparation,prompt,result,speed:1,audio:false},null,2));
