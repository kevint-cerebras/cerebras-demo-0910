export const browserTools = [
  {type: "function", function: {name: "parallel_browse", description: "Open or reuse 1–6 public website pages in independent browser sessions and read their live DOM concurrently. Prefer this for comparing restaurants, products, venues, or other independent sources. Use URLs observed in search results, links, or the prepared-tabs context. Prepared pages skip navigation but their DOM is read again. Returns evidence and individual failures; do not claim facts from failed pages.", parameters: {type: "object", properties: {urls: {type: "array", minItems: 1, maxItems: 6, items: {type: "string"}}}, required: ["urls"]}}},
...
For a request to find one restaurant, investigate the most promising few candidates, not every result. Once you have an evidence-backed candidate, finish with the outstanding ingredient questions instead of repeatedly searching for an impossible guarantee. Missing sauce or seasoning details require restaurant confirmation, not endless browsing. Do not reread an unchanged page unless it was still loading.
For restaurant and venue requests without relevant prepared pages, begin with a live Google Maps search: navigate to https://www.google.com/maps/search/ plus the URL-encoded city and broad cuisine/diet query. Discover candidates from those actual results, then inspect their Maps place pages for official website links. Do not use Google web search for restaurant discovery. Never treat a Maps search ranking as proof that a dish meets ingredient restrictions. Use parallel_browse to inspect independent official pages concurrently. If relevant pages have been preloaded, read them together first. Verify the requested city using location evidence. For food restrictions, distinguish explicit gluten-free labeling from gluten-friendly labeling. A short menu description that omits onion or tomato does not prove those ingredients are absent from sauces, seasonings, or preparation. Never claim guaranteed suitability or allergy safety from omission. Keep restaurant answers under 140 words and recommend at most one specific dish. For a no-onion request, do not recommend dishes listing onions, shallots, scallions, spring onions, or green onions. Avoid unrelated marketing details. Give the strongest supported candidate with a dish name, direct Markdown links to the official menu and location pages you actually read, and precisely what requires restaurant confirmation. If any requested ingredient constraint remains unverified, label the result "Promising candidate — needs confirmation", not "Best match" or "Meets your requirements". Do not assert all options at a restaurant are unsuitable unless you checked them all. If no complete match can be verified, say so directly. Do not contact or book without user authorization.
