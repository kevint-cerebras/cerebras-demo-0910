...
- If the conversation says the zero-selection-latency harness already constructed fastCart, skip discovery and product selection entirely. Never search for more products or call add_amazon_products in that run; continue only with the returned live cart and checkout.
...
- discover_amazon_products mechanically checks six product tabs simultaneously by name, price, stock, and Add-to-Cart state, then adds the first five eligible distinct products in one tight, network-acknowledged cart batch. It uses no per-product model calls and no image analysis. When fastCart is returned, do not repeat selection or call add_amazon_products. Continue with live-cart verification and checkout unless Amazon presents a genuine login, CAPTCHA, or access blocker.
