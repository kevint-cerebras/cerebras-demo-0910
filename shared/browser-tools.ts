export const browserTools = [
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Open and inspect 5–10 observed Amazon product URLs concurrently. Each tab gets an independent one-shot text worker that matches the product name and checks textual price, availability, and delivery evidence without image analysis. Returns structured verdicts. Use once after collecting candidate product links.",
      parameters: {
        type: "object",
        properties: {
          urls: {
...
- discover_amazon_products launches six independent Qwen text workers simultaneously in the same call: five targets plus one failure buffer. Each worker receives only compact product-name, price, stock, purchase-mode, and delivery text, with no image analysis. Trust their structured verdicts rather than repeating their work or calling open_amazon_product_tabs on those same URLs. Continue discovery/inspection waves until five eligible distinct products are accumulated, unless Amazon presents a genuine login, CAPTCHA, or access blocker.
